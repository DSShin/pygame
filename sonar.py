...
def getNewBoard():
    ...
    for x in range(60): # the main list is a list of 60 lists
        ...
        for y in range(15): # each list in the main list has 15 single character strings
            # user different characters for the ocean to make it more readable.
...
def makeMove(board, chests, x, y):
    ...
    # from the chests list as they are found. Return False if this is an invlid move.
    ...
    if smallestDistance == 0:
        ...
        if smallestDistance < 10:
            ...
            return 'Treasure detected at a distance of %s from the sonar devie.' % (smallestDistance)
        else:
            board[x][y] = '0'
...
def playAgain():
    # This function returns True if the player wants to play againg, otherwise it returns False.
...
is to find the three sunken treaure chests that are lurking in the part of the
...
Press enter to continue...''')
    ...
    print('''For example, here is a trasue chest (the c) located a distance of 2 away
...
the location of the chest, and it will be colleted. The sonar device will
remain there.

When you collect a chest, all sonar devices wipp update to locate the next
...
while True:
    ...
    while sonarDevices > 0:
        ...
        print('You have %s sonar device%s left. %s treasure chest%s remainin.' % (sonarDevices, extraSsonar, len(theChests), extraSchest))

        x, y = enterPlayerMove()

        previousMoves.append([x,y]) # we must track all moves so that sonar devies can be updated.
        ...
        if moveResult == False:
            ...
            sonarDevices -= 1
        if sonarDevices == 0:
            print('We\'ve run out of sonar devies! Now we have to turn the ship around and head')
